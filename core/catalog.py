...
# Los ejes de 'clean_artifacts'. Los valores son las claves que entiende
...
HEAVY_AXIS_VALUES = ['node_modules', 'venv', 'build', 'dart_tool']
HEAVY_AXIS_LABELS = {'node_modules': 'node_modules', 'venv': '.venv',
                     'build': 'build/dist', 'dart_tool': '.dart_tool'}
...
_BUMP_AXIS = lambda: AxisDef('bump_mode', BUMP_MODES, 'scope', label='Bump',
                             combine={'build'}, default='patch', join='+')
...
# Los mismos tres pasos de Flutter, con la misma semantica: 'Compilar SPA'
...
BUILD_BINARY_STEPS = [
    Step('bump', 'Subir número de versión'),
    Step('build', 'Compilar binario'),
    Step('checksum', 'Calcular checksum'),
    Step('upload', 'Subir al VPS', default=False, requires_env=_VPS_KEYS),
]
...
PUBLISH_CODE_STEPS = [
    Step('push', 'Empujar local a GitHub'),
    Step('pull', 'Actualizar VPS',
         requires_env={'GIT_REPO_URL', 'VPS_DEPLOY_DIR'}),
    Step('deps', 'Instalar dependencias',
         requires_env={'VPS_PYTHON', 'SERVER_DIR'}),
    Step('upload', 'Copiar archivos ignorados'),
]
...
#
# Cuales extensiones son sale de `DB_EXTENSIONS`, no se deduce. Salian de los
# `CREATE EXTENSION` de las migraciones del repo y eso no podia funcionar:
# `alembic revision --autogenerate` nunca escribe uno, asi que la lista salia
# vacia; y una migracion que si lo declarara es la que necesita la extension ya
# creada para poder aplicarse. La casilla sigue eligiendo si el paso corre; el
# QUE no es un parametro de la corrida sino configuracion del repo, y por eso no
# tiene eje: no cambia entre dos veces que se aprieta el boton.
#
...
SETUP_SSH_STEPS = [
    Step('access', 'Crear cuenta de despliegue',
         requires_env={'VPS_IP', 'VPS_KEY_NAME'}),
    Step('sudo', 'Configurar sudo sin contraseña', requires_env={'VPS_IP'}),
    Step('verify', 'Verificar login por SSH',
         requires_env={'VPS_IP', 'VPS_KEY_NAME'}),
]
...
SETUP_GITHUB_SSH_STEPS = [
    ...
    Step('register', 'Registrar llave en GitHub',
         requires_env={'GITHUB_TOKEN', 'GITHUB_KEY_TITLE'}),
    Step('verify', 'Verificar acceso a GitHub'),
]

# El reverso del anterior, y los dos lados son desmarcables por separado: una
# llave puede quedar dada de baja en GitHub y seguir en el disco del VPS.
REVOKE_GITHUB_SSH_STEPS = [
    Step('remote_files', 'Borrar llave del VPS'),
    Step('github_side', 'Dar de baja llave en GitHub',
         requires_env={'GITHUB_TOKEN', 'GITHUB_KEY_TITLE'}),
]

# Los dos lados de la clave con la que entra ESTA maquina. Desmarcar el local
# es el caso de revocar el acceso de una maquina que ya no se tiene enfrente.
REVOKE_SSH_STEPS = [
    Step('remote_side', 'Borrar clave del VPS'),
...
INSTALL_ANDROID_SDK_STEPS = [
    ...
    Step('hypervisor', 'Instalar aceleración del emulador'),
]

# `teardown_db` era `level='C'` sin pasos: sus dos booleanos no tenian casilla y
# borraba siempre las dos cosas. Borrar la base y dejar el rol es lo que se
# quiere antes de un bootstrap: el rol lo usan otras bases del mismo servidor.
TEARDOWN_DB_STEPS = [
    Step('database', 'Borrar base de datos'),
...
DEV_ENV_STEPS = [
    ...
    Step('serve_vite', 'SPA'),
    Step('run_python', 'App de escritorio', default=False),
]
...
def load_catalog() -> None:
    ...
    # --- Ejecutar ------------------------------------------------------------
    # Lo que el launcher TE ENTREGA decide si su pestana tiene segunda vista:
    # una URL se mira en un navegador, un emulador se mira en su ventana
    # (ADR-0008).
    #
    ...
    registry.register(Capability(
        id='backend', name='Levantar backend', group='Ejecutar',
        ...
        axes=[AxisDef('target', [], 'scope', label='Servidor', discover=(targets.FASTAPI,)),
              ...
              AxisDef('reload', ['con recarga', 'sin recarga'], 'scope',
                      ...
                      labels={'con recarga': 'Con recarga', 'sin recarga': 'Sin recarga'})],
        stub=True))
    ...
    registry.register(Capability(
        id='run_mobile', name='Levantar app móvil', group='Ejecutar',
        kind='live', icon='📲',
    ...
    # cada una (ADR-0010).
    registry.register(Capability(
        id='run_python', name='Levantar app de escritorio', group='Ejecutar',
    ...
    registry.register(Capability(
        id='build_binary', name='Compilar ejecutable', group='Compilar', kind='once', icon='⚡',
        ...
        axes=[AxisDef('entrypoint', [''], 'field', label='Punto de entrada'),
              ...
              AxisDef('onefile', ['un archivo', 'carpeta'], 'scope',
                      label='Empaquetado', truthy='un archivo',
                      labels={'un archivo': 'Un archivo', 'carpeta': 'Carpeta'}),
              AxisDef('windowed', ['consola', 'ventana'], 'scope',
    ...
    registry.register(Capability(
        id='promote_app', name='Publicar en producción', group='Compilar',
        ...
        stub=True))
    registry.register(Capability(
        id='clean_artifacts', name='Borrar artefactos de build', group='Compilar',
        ...
        axes=[
            ...
            AxisDef('heavy', HEAVY_AXIS_VALUES, 'checks', select='many', label='Pesados',
                    labels=HEAVY_AXIS_LABELS, checked_by_default=False, allow_empty=True),
            AxisDef('apply', ['simulacro', 'borrar'], 'scope', label='Modo',
    ...
    registry.register(Capability(
        id='clone_repo', name='Clonar repositorio de GitHub', group='Repositorio',
    ...
    registry.register(Capability(
        id='sync_server_env', name='Igualar claves de config.env y .env', group='Repositorio', kind='destructive', icon='🔃',
    ...
    registry.register(Capability(
        id='enable_extensions', name='Habilitar extensiones', group='Base de datos', kind='once', icon='🧩',
    ...
    registry.register(Capability(
        id='reinit_migrations', name='Reiniciar historial de migraciones', group='Base de datos', kind='destructive', icon='🧨',
    ...
    registry.register(Capability(id='teardown_db', name='Borrar base y rol', group='Base de datos', kind='destructive', icon='💥', description='Borra la base de la aplicación y su rol.', axes=[_SCOPE_AXIS()], steps=TEARDOWN_DB_STEPS, stub=True))
    ...
    registry.register(Capability(
        id='setup_ssh_key', name='Crear usuario y acceso SSH', group='VPS',
        kind='once', icon='🔑',
    ...
    registry.register(Capability(
        id='install_system_image', name='Instalar imagen de sistema', group='Emuladores', kind='once', icon='💿', scope='machine',
    ...
    registry.register(Capability(
        ...
        stub=True))
    registry.register(Capability(
        id='launch_emulator', name='Abrir emulador', group='Emuladores', kind='live', icon='📲', scope='machine',
        ...
        axes=[AxisDef('avd', [], 'pick', label='AVD', source=ANDROID_AVDS),
              AxisDef('wipe', ['normal', 'borrar datos'], 'scope', label='Arranque',
                      ...
                      labels={'normal': 'Normal', 'borrar datos': 'Borrar datos'}),
              # Los tres flags que de verdad se escriben. Eran texto libre
              # —habia que acordarse de como se llaman y escribirlos bien— y
              # son una eleccion de si o no, que es exactamente una casilla.
              # El nombre del flag ES el valor: asi no hace falta una tabla
              # que traduzca, y `core/tasks/emulators.py` los suma tal cual.
              AxisDef('boot_flags', ['-no-snapshot-load', '-no-snapshot-save', '-no-window'],
                      'checks', select='many', checked_by_default=False, allow_empty=True,
                      label='Opciones de arranque',
                      labels={'-no-snapshot-load': 'Arranque en frío (ignora el snapshot)',
                              '-no-snapshot-save': 'No guardar el snapshot al cerrar',
                              '-no-window': 'Sin ventana (headless, solo adb)'}),
              # El renderizado es excluyente, no una casilla: pedir host y
              # software a la vez no significa nada.
              AxisDef('gpu', ['host', 'software'], 'scope', label='GPU',
                      labels={'host': 'Host (GPU real)',
                              'software': 'Software (swiftshader)'}),
              # Lo que queda de la linea de comandos, para el flag que no esta
              # arriba (`-memory 4096`, `-http-proxy ...`). Se suma a las
              # casillas y a los flags de siempre de `core/android.py`
    ...
    registry.register(Capability(
        ...
        # funcion. La declaracion es el contrato, la haya dibujado alguien o no.
        axes=[AxisDef('serial', [], 'pick', label='Emulador', source=ANDROID_RUNNING)],
    ...
    # --- SDKs ----------------------------------------------------------------
    registry.register(Capability(
        id='install_android_sdk', name='Instalar SDK de Android', group='SDKs',
        ...
        axes=[AxisDef('install_dir', [ANDROID_DIR_DEFAULT], 'field', label='Directorio'),
              *ANDROID_PACKAGE_AXES],
        steps=INSTALL_ANDROID_SDK_STEPS, stub=True))
    ...
    registry.register(Capability(
        ...
        stub=True))
    registry.register(Capability(
        id='install_android_packages', name='Instalar paquetes del SDK', group='SDKs', kind='once', icon='📦', scope='machine',
        description='Acepta las licencias e instala en el SDK los paquetes que falten.',
        axes=list(ANDROID_PACKAGE_AXES), stub=True))
    registry.register(Capability(
        id='install_android_hypervisor', name='Instalar aceleración del emulador', group='SDKs', kind='once', icon='⚡', scope='machine',
