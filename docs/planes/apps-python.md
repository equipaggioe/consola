...
> **Estado (verificado 2026-09-21).** Construido: la **pista A**, de otra forma que la que este
> documento propone — sin tabla `[tool.consola]`, con la inferencia de `core/targets.py`, y con el
> servidor en su propio botón (`backend`) en vez de fundido en `run_python`. `run_python` ya es un
> launcher con eje descubierto, `fanout` y vigilante de fuentes
> ([ADR-0010](../adr/0010-fanout-o-bucle-segun-kind.md)).
>
> Pendiente: la **pista B** —el eje `many` descubierto de `build_binary`, que hoy es un campo de
> texto— y la atómica `install_python_deps`, que no existe. También sigue abierta la decisión del
> §3.3 (el repo que es su propia app): hoy `resolve_entrypoint` cae en la raíz del repo, que es la
> opción (b) sin haber elegido. Ver [arquitectura 30](../arquitectura/30_riesgos_y_pendientes.md).
>
> Donde este plan y el código no coincidan, manda [`docs/arquitectura/`](../arquitectura/README.md).
...
- `select='many'` + `kind='live'` → **una pestaña por app**, por la regla de
  [ADR-0010](../adr/0010-fanout-o-bucle-segun-kind.md). Es exactamente lo que se quiere para levantar backend y
...
Aplicando el criterio de [ADR-0004](../adr/0004-un-boton-por-capacidad.md) (*¿tiene sentido re-ejecutar esto solo?*):
...
- `select='many'` + `kind='once'` → **bucle en una sola consola**, N binarios en fila, por la misma
  regla de [ADR-0010](../adr/0010-fanout-o-bucle-segun-kind.md) que gobierna `build_vite`. Hoy es imposible: hay que
...
| `docs/arquitectura/`, `docs/adr/` | arquitectura del dominio y decisiones nuevas |
