...
"""
Ejecuta seeders de datos de prueba.
...
- Descubre funciones async que empiecen con seed_ dentro de mock_data.
...
async def run() -> None:
    ...
    try:
        ...
        log("INFO", "Iniciando seeders mock: mock_data")
        try:
            seeds = _discover_seed_functions("mock_data")
        except ModuleNotFoundError:
            log("WARN", "No existe el paquete mock_data. Saltando.")
            return

        if not seeds:
            log("INFO", "No se encontraron funciones seed_ en mock_data.")
            return
        ...
        try:
            async with session_factory() as db:
                ...
                await db.commit()
            log("OK", "Seeders mock completados.")
        finally:
